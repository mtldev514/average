...
// ─── Build stats from model ────────────────────────────

const MODEL = basicModel as ModelData;
        ...
        return (
          <button
            ...
            style={{
              ...
              fontSize: 11,
              padding: "3px 10px",
              border: `1px solid ${active ? stat.color : "#ddd8d0"}`,
              borderRadius: 12,
              background: active ? `${stat.color}18` : "transparent",
              color: active ? stat.color : "#aaa",
              cursor: "pointer",
...
function Summary({ results }: { results: Record<string, number | null> }) {
  ...
  return (
    <div style={{ marginTop: 64, paddingTop: 40, borderTop: "2px solid #1a1a1a", animation: "fadeIn 0.6s ease" }}>
      <div style={{ fontSize: 11, color: "#aaa", fontFamily: "'IBM Plex Mono', monospace", letterSpacing: 3, marginBottom: 16, textAlign: "center" }}>
        SCORE COMPOSITE &middot; {filled.length} / {STATS.length}
      </div>
    ...
    <div style={{ minHeight: "100vh", background: "#f5f2ec", padding: "60px 24px 120px" }}>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .stat-slider::-moz-range-track {
          height: 4px; border-radius: 2px; background: #e8e4de;
        }
        .meta-btn {
      ...
      <div style={{ maxWidth: 720, margin: "0 auto" }}>
        ...
        <div style={{ marginBottom: 40 }}>
          <h1 style={{
            fontFamily: "'DM Serif Display', Georgia, serif",
            fontSize: "clamp(48px, 9vw, 72px)", fontWeight: 400,
            color: "#1a1a1a", lineHeight: 1, letterSpacing: "-2px",
          }}>
            {MODEL.model}.
          </h1>
          <p style={{
            fontFamily: "'IBM Plex Mono', monospace", fontSize: 13,
            color: "#999", marginTop: 14, lineHeight: 1.7, maxWidth: 480,
          }}>
            {MODEL.philosophy}
          </p>
        ...
        {/* Footer */}
        <div style={{ marginTop: 64, fontSize: 10, color: "#ccc", fontFamily: "'IBM Plex Mono', monospace", letterSpacing: 1.5, textAlign: "center", lineHeight: 2 }}>
          MOD&Egrave;LE: {MODEL.model.toUpperCase()} &middot; {STATS.length} QUESTIONS &middot; DISTRIBUTIONS APPROXIMATIVES
        </div>
      </div>
    </div>
  );
